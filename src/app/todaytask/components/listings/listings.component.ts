...
export class TodayListingComponent implements OnInit {
  myControl = new FormControl();

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns = ['service_name', 'captain', 'property_name', 'first_name','service_completed','created_at','user_profile_service_day'];
  allResidentUsers = [];
  allbuilding = [];
  allbuildingfilter = [];
  allTasks: any;
...
constructor(private router: Router, breakpointObserver: BreakpointObserver, private service: TasksService) {
  ...
  $('.page-title').text("Today's Tasks");
  breakpointObserver.observe(['(max-width: 600px)']).subscribe(result => {
    this.displayedColumns = result.matches ?
      [ 'service_name', 'resident', 'property_name', 'captain','status','created_at','user_profile_service_day'] :
      [ 'service_name', 'resident', 'property_name', 'captain','status','created_at','user_profile_service_day'];
  });
}
  ngOnInit() {
